...
describe('POST /consent', () => {
  ...
  it('should fail when request is malformed', async () => {
    let result = await chai.request(server.app).post('/consent').send({
      accessCode: currentAccessCode.value,
    });
    result.should.have.status(400);

    result = await chai.request(server.app).post('/consent').send({
      consent: true,
    });
    result.should.have.status(400);
  });
  ...
  it('should fail when access code does not exist', async () => {
    const result = await chai.request(server.app).post('/consent').send({
